...
async def test_term_crud(farm_auth):
    hostname, auth = farm_auth
    async with AsyncFarmClient(hostname, auth=auth) as farm:
        post_response = await farm.term.send(test_term["type"], test_term["payload"])
        ...
        # Get the term by ID.
        get_response = await farm.term.get_id(test_term["type"], test_term["id"])
        ...
        # Update the term.
        patch_response = await farm.term.send(test_term["type"], test_term_changes)
        # Get the term by ID.
        get_response = await farm.term.get_id(test_term["type"], test_term["id"])
        ...
        # Delete the term.
        deleted_response = await farm.term.delete(test_term["type"], test_term["id"])
        assert deleted_response.status_code == 204


@pytest.mark.anyio
@farmOS_testing_server
async def test_term_get(farm_auth, test_terms):
    hostname, auth = farm_auth
    async with AsyncFarmClient(hostname, auth=auth) as farm:
        # Get one page of plant_type terms.
        response = await farm.term.get(test_term["type"])
        assert "data" in response
        assert "links" in response
        assert len(response["data"]) == 50

        # Get all plant_type terms.
        all_terms = [term async for term in farm.term.iterate(test_term["type"])]
